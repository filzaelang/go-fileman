...
func GetBUList(payload BuChildList) ([]BUList, error) {
	var listBU []BUList
	var err error
	var queryBUAlreadyExist string

	if payload.Type == BUFOLDER {
		queryBUAlreadyExist = `select distinct divoid, seq
		from bu_list
		where divoid in (
	...
	listBU, err = BUListFolder(payload, queryBUAlreadyExist)

	return listBU, err
}

func BUListFolder(payload BuChildList, queryBUAlreadyExist string) ([]BUList, error) {
	var listBU []BUList
	...
	// Fetch only where divoid doesnt exist
	placeholders := ""
	args := []interface{}{}
...
// func BUListBUdeptFolder(payload BuChildList) ([]BUList, error) {
// 	var listBU []BUList
